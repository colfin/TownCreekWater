...
# This is actually a better way to make the plot anyway
...
```{r "Mock" style NOX plot}
# This is actually a better way to make the plot anyway

# Make $Date a Date object:
NOX_Unknown_dropNegs$Date <- as.Date(NOX_Unknown_dropNegs$Date)

plot.Mock.NOX <- ggplot(data = NOX_Unknown_dropNegs, aes(x = Date, y = Peak.Concentration, shape=Site, color=Site, fill = Site)) +
  geom_line(size = 1) +
  geom_point(size = 4)+
  scale_color_manual(values=c("#CC79A7","#0072B2"), labels = c("Upstream", "Downstream"), name="Sample Location") +
  scale_shape_manual(values = c(24, 25), labels = c("Upstream", "Downstream"), name = "Sample Location")+
...
```{r two-panel "Mock" style}
two_panel_NH3_NOX_V2 <- ggarrange(plot.Mock, plot.Mock.NOX, ncol = 1, nrow = 2, labels = NULL, legend = "none")

two_panel_NH3_NOX_V2

#ggsave("../figures/NH3_NOX.png", plot=two_panel_NH3_NOX_V2, device="png", path=NULL, scale=1, width=12, height=10, dpi=600, limitsize=TRUE, bg="white")
```
